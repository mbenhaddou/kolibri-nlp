...
def slice_bounds(sequence, slice_obj, allow_step=False):
    ...
    # Otherwise, make sure that no non-default step text is used.
...
class AbstractLazySequence(object):
    ...
    dataset view objects, which provide access to the contents of a
    dataset without loading the entire dataset into memory, by loading
    pieces of the dataset from disk as needed.
    ...
    sequence caches that element's text or reconstructs it from
    scratch.
    ...
    def __len__(self):
        """
        Return the number of tokens in the dataset file underlying this
        dataset view.
        """
    ...
    def iterate_from(self, start):
        """
        Return an iterator that generates the tokens in the dataset
        file underlying this dataset view, starting at the token number
    ...
    def __getitem__(self, i):
        """
        Return the *i* th token in the dataset file underlying this
        dataset view.  Negative indices and spans are both supported.
        """
    ...
    def __iter__(self):
        """Return an iterator that generates the tokens in the dataset
        file underlying this dataset view."""
        return self.iterate_from(0)

    def count(self, value):
        """Return the number of times this list contains ``text``."""
        return sum(1 for elt in self if elt == value)

    def index(self, value, start=None, stop=None):
        """Return the index of the first occurrence of ``text`` in this
    ...
    def __contains__(self, value):
        """Return true if this list contains ``text``."""
    ...
    def __repr__(self):
        """
        Return a string representation for this dataset view that is
        ...
        for sublist in sublist_iter:
            if sublist_index == (len(self._offsets) - 1):
                ...
                assert self._offsets[sublist_index + 1] == index + len(
                    sublist
                ), 'inconsistent list text (num elts)'
...
class LazyMap(AbstractLazySequence):
    ...
    function is applied lazily -- i.e., when you read a text from the
    list, ``LazyMap`` will calculate that text by applying its
    function to the underlying lists' text(s).  ``LazyMap`` is
    ...
    equal size, then the text None will be supplied for the
    'missing' elements.
    ...
    case with many dataset readers.
    A typical example of a use case for this class is performing
    feature detection on the tokens in a dataset.  Since featuresets
...
class LazyZip(LazyMap):
    ...
    tuples are constructed lazily -- i.e., when you read a text from the
    list, ``LazyZip`` will calculate that text by forming a tuple from
...
class LazyEnumerate(LazyZip):
    ...
    zero) and a text yielded by underlying sequence.  ``LazyEnumerate`` is
    useful for obtaining an indexed list. The tuples are constructed lazily
    -- i.e., when you read a text from the list, ``LazyEnumerate`` will
    calculate that text by forming a tuple from the count of the i-th
...
class LazyIteratorList(AbstractLazySequence):
    ...
    def iterate_from(self, start):
        """Create a new iterator over this list starting at the given start."""
...
class Trie(dict):
    ...
    def insert(self, string):
        ...
        :type string: str
        :Example:
        >>> from kolibri.utils.collections import Trie
        >>> trie = Trie(["abc", "def"])
