...
from kolibri.utils._collections import AbstractLazySequence, LazyConcatenation, LazySubsequence
from kolibri.utils.file import *
from kolibri.utils.file import ZipFilePathPointer
######################################################################
# { Corpus View
######################################################################
...
class StreamBackedCorpusView(AbstractLazySequence):
    """
    A 'view' of a dataset file, which acts like a sequence of tokens:
    it can be accessed by index, iterated over, etc.  However, the
    tokens are only constructed as-needed -- the entire dataset is
    never stored in memory at once.
    The constructor to ``StreamBackedCorpusView`` takes two arguments:
    a dataset fileid (specified as a string or as a ``PathPointer``);
    ...
    dataset, careful consideration should be given to the size of
    blocks handled by the block reader.  Smaller block sizes will
    increase the memory requirements of the dataset view's internal
    data structures (by 2 integers per block).  On the other hand,
    larger block sizes may decrease performance for random access to
    the dataset.  (But note that larger block sizes will *not*
    ...
    have high degrees of locality, the dataset view may cache one or
    more blocks.
    :note: Each ``CorpusView`` object internally maintains an open file
        object for its underlying dataset file.  This file should be
    ...
    :ivar _filepos: A list containing the file position of each block
        ...
        indices and file positions.
    :ivar _stream: The stream used to access the underlying dataset file.
    :ivar _len: The total number of tokens in the dataset, if known;
        or None, if the number of tokens is not yet known.
    :ivar _eofpos: The character position of the last character in the
        file.  This is calculated when the dataset view is initialized,
    ...
    def __init__(self, fileid, block_reader=None, startpos=0, encoding='utf8'):
        """
        Create a new dataset view, based on the file ``fileid``, and
        read with ``block_reader``.  See the class documentation
        for more information.
        :param fileid: The path to the file that is read by this
            dataset view.  ``fileid`` can either be a string or a
            ``PathPointer``.
    ...
    def _open(self):
        """
        Open the file stream associated with this dataset view.  This
        will be called performed if any text is read from the view
    ...
    def close(self):
        """
        Close the file stream associated with this dataset view.  This
        can be useful if you are worried about running out of file
        handles (although the stream should automatically be closed
        upon garbage collection of the dataset view).  If the dataset
...
class ConcatenatedCorpusView(AbstractLazySequence):
    """
    A 'view' of a dataset file that joins together one or more
    ...
    def __init__(self, corpus_views):
        ...
        """A list of the dataset subviews that make up this
        concatenation."""
...
def concat(docs):
    ...
    single dataset, using an appropriate concatenation function.  This
    utility function is used by dataset readers when the user requests
    ...
    # If they're all dataset views, then use ConcatenatedCorpusView.
    for typ in types:
...
class PickleCorpusView(StreamBackedCorpusView):
    """
    A stream backed dataset view for dataset files that consist of
    sequences of serialized Python objects (serialized using
    ``pickle.dump``).  One use case for this class is to store the
    result of running feature detection on a dataset to disk.  This can
    be useful when performing feature detection is expensive (so we
    don't want to repeat it); but the dataset is too large to store in
    memory.  The following example illustrates this technique:
        >>> from kolibri.dataset.utils import PickleCorpusView
        >>> from kolibri.utils.collections import LazyMap
        >>> feature_corpus = LazyMap(detect_features, dataset) # doctest: +SKIP
    ...
    def __init__(self, fileid, delete_on_gc=False):
        """
        Create a new dataset view that reads the pickle dataset
        ``fileid``.
    ...
    def __del__(self):
        ...
        ``PickleCorpusView`` was created, then delete the dataset view's
    ...
    def cache_to_tempfile(cls, sequence, delete_on_gc=True):
        ...
        dataset; and then return a ``PickleCorpusView`` view for that
...
def read_csv_line_block(stream):
    toks=[]
...
def _path_from(parent, child):
    ...
    while parent != child:
        ...
        if len(os.path.split(child)[0])>0:
