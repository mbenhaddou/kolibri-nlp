...
from kolibri.dataset.corpusreader import *
from kolibri.tokenizer import WordTokenizer
from kolibri.tokenizer import SentenceTokenizer
from kolibri.dataset.utils import read_blankline_block
...
class PlaintextCorpusReader(CorpusReader):
    ...
    This dataset reader can be customized (e.g., to skip preface
    sections of specific document formats) by creating a subclass and
    overriding the ``CorpusView`` class variable.
    """

    CorpusView = StreamBackedCorpusView
    """The dataset view class used by this reader.  Subclasses of
       ``PlaintextCorpusReader`` may specify alternative dataset view
    ...
    def __init__(
        ...
        """
        Construct a new plaintext dataset reader for a set of documents
    ...
    def sents(self, fileids=None):
        ...
        if self._sent_tokenizer is None:
            raise ValueError('No sentence tokenizer for this dataset')
    ...
    def yield_sents(self, fileids=None):
        if self._sent_tokenizer is None:
            raise ValueError('No sentence tokenizer for this dataset')
        ...
        for (path, enc, fileid) in self.abspaths(fileids, True, True):
            ...
            yield self.CorpusView(path, self._read_sent_block, encoding=enc)
    ...
    def _read_sent_block(self, stream):
        ...
        for para in self._para_block_reader(stream):
            sents.extend(
                [
                    self._sent_tokenizer.tokenize(para)
                ]
            )
        return sents
...
class CategorizedPlaintextCorpusReader(CategorizedCorpusReader, PlaintextCorpusReader):
    ...
    def __init__(self, *args, **kwargs):
        """
        Initialize the dataset reader.  Categorization arguments
...
class EuroparlCorpusReader(PlaintextCorpusReader):
    ...
    that:
      - Since the dataset is pre-processed and pre-tokenized, the
    ...
    def paras(self, fileids=None):
        raise NotImplementedError(
            'The Europarl dataset reader does not support paragraphs. Please use chapters() instead.'
        )
