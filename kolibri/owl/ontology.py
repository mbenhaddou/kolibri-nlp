...
from kolibri.owl.entities import *
...
class Ontology:
    ...
    def load(self, location=None, format=None, **args):
        """
        ...
        self._parse(location, format, **args)

        self.sync_from_graph()

    def _parse(self, location=None, format=None, **args):
        """
        ...
        #hacky, hacky, hacky
        try:
            self.graph.parse(location, format=format, **args)
            ...
            for fmt in formats:
                try:
                    self.graph.parse(location, None, fmt, None, None, **args)
                    return
                except Exception:
                    pass
    ...
    def _load_classes(self):
        ...
        uris = set(uri for uri in self.graph.subjects(RDF.type, OWL.Class)) \
               | set(uri for uri in self.graph.subjects(OWL.subClassOf, None))
    ...
    def _load_object_properties(self):
        ...
        uris.extend([uri for uri in self.graph.subjects(RDF.type, RDF.Property)])
    ...
    def get_super_classes(self, cls):
        ...
        if len(parent_uris)==0:
            parent_uris = set(self.graph.objects(cls.uri, OWL.subClassOf))
    ...
    def get_sub_classes(self, cls):
        ...
        if len(children_uris)==0:
            children_uris = set(self.graph.subjects(OWL.subClassOf, cls.uri))
    ...
    def get_class(self, uri=None, match=None):
        """
        ...
        if match:
            ...
            if ":" in match:  # qname
                for x in self.classes:
                    if match.lower() in x.qname:
                        res += [x]
            else:
                for x in self.classes:
                    if rdflib.term.Literal(match) in x.labels:
                        res += [x]
                    elif match in x.uri.split('/'):
                        res += [x]
            return res
