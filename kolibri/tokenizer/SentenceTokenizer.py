...
"""
EMAIL_STARTERS=compile(r"(?P<sentence1>.+)?(?P<sentence2>\b(Hi|Hello|Dear|RE:)\b\s?(\w+\s?){1,2}\s?[,-]?(.+))",re.IGNORECASE)
...
def split_single(text, join_on_lowercase=False, short_sentence_length=SHORT_SENTENCE_LENGTH):
    ...
    """
    sentences_ = _sentences(DO_NOT_CROSS_LINES.split(text), join_on_lowercase, short_sentence_length)
    sentences_= [s for ss in sentences_  for s in split_newline(ss)]

    sentences=[]
    for sentence in sentences_:
        match=EMAIL_STARTERS.match(sentence)
        if match:
            if match['sentence1'] is not None:
             sentences.append(match['sentence1'])
            if match['sentence2'] is not None:
             sentences.append(match['sentence2'])
        else:
